...
def conv_layer(input_dim, output_dim):
    return nn.Sequential(
        nn.Conv2d(input_dim, output_dim, kernel_size= (3, 3), stride=1),  # input shape - 2
        #nn.GroupNorm(num_groups=output_dim, num_channels=output_dim),
        nn.LeakyReLU(0.2)
    )
...
class Discriminator(nn.Module):
    def __init__(self, im_dim=3, hidden_dim=4, input_shape=(256, 256)):
        ...
        self.model = nn.Sequential(
            ...
            conv_layer(hidden_dim * 4, hidden_dim * 2),  # -6
            nn.Flatten(),
            nn.Linear(in_features=2015744, out_features=100),
            nn.Sigmoid(),
