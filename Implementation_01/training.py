...
cycle_lambda, identity_lambda, gen_lambda = 2, 1, 2
...
def get_gen_loss(disc, fake, criterion):
    discFake = disc(fake)
    loss = criterion(discFake, torch.ones_like(discFake).to(device))
    return loss


def get_identity_loss(y, yhat, criterion):
...
for epoch in range(NumEpochs):
    for i, (KittiData, CarlaData) in enumerate(zip(KittiDataLoader, CarlaDataLoader), 0):
        ...
        Real2SimGeneratorLoss = gen_lambda * get_gen_loss(disc=Real2SimDiscriminator,
                                                          fake=simFake, criterion=advCriterion)
        ...
        if i % 100 == 0:
            print("at step {} of epoch {}, total Generator loss is {} and the total Discriminator loss is {}".format(
                i, epoch, TotalGenLoss, Real2SimDiscLoss+Sim2RealDiscLoss))
        else:
